...
#if defined(NDEBUG)
# error "Suc Core cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in nuffs) are considered zero fee (for relaying, mining and transaction creation) */
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
	...
	} else if(nPrevHeight < 64800 && nPrevHeight > 0) {
        nSubsidyBase = 10;
	} else if (nPrevHeight >= 64800  && nPrevHeight < 129600) {
		nSubsidyBase = 15;
	} else if (nPrevHeight >= 129600 && nPrevHeight < 262800) {
		nSubsidyBase = 20;
	} else {
		nSubsidyBase = 10;
	}
	...
	// yearly decline of production by 10% per year until reached max coin ~29.3M.
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
	...
	if(nHeight <= 64800) {
        ret = blockValue * 0.8;
	} else if (nHeight > 64800  && nHeight <= 129600) {
        ret = blockValue * 0.7;
	} else {
        ret = blockValue * 0.6;
	} 
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // SUC : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(SUC): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(SUC): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // SUC : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(SUC): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(SUC): spork is off, skipping transaction locking checks\n");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        Suc Related Inventory Messages
