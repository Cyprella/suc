...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "SucNetwork") type = SucNetwork;
    if(strType == "SucNetworkVariable") type = SucNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == SucNetwork) s = "SucNetwork";
    if(type == SucNetworkVariable) s = "SucNetworkVariable";
